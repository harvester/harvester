...
status:
    ...
    checks:
        VMHostDeviceLiveMigrationUnsupported:
            severity: Warning
            message: "150 VMs cannot be live migrated: host devices or vGPU devices attached"
            affectedCount: 150
            truncated: true
            affectedResources:
...
`checks` is a map keyed by `reason`, so a controller can update or delete a single check result when a resource changes or when a warning returns to normal. `affectedResources.names` is a map keyed by resource name. It uses `namespace/name` for namespaced resources and bare `name` for cluster-scoped resources. The map value is currently empty and reserved for future per-resource detail. When the number of affected resources exceeds a threshold (recommended: 20), `names` is truncated to that limit and `truncated: true` is set to signal to the dashboard that more exist.

`affectedResources` is a pointer field and may be absent for checks that do not relate to a specific resource kind. A check is expected to contain one `affectedResources` group because affected resources for the same reason are normally the same `apiVersion` and `kind`.
...
`components` is a map keyed by component name so the summary controller can update one component's counters without rebuilding a positional list. The summary CR does not embed the check details from each component. Its only role is to give the dashboard a single read to determine per-component error and warning counts. Detail is always fetched from the individual `ComponentHealth` CR.
...
- **Resource configuration that blocks expected operations** — a resource exists but its current configuration prevents a normal operation from succeeding (e.g. a VM uses host devices or vGPU devices and cannot be live-migrated). There is no deletion webhook for this case; the health controller detects and surfaces it proactively.
...
The health controller injects the same shared `Validator` and builds a reason-keyed `CheckResult` from the structured output. Severity, reason key, and resource metadata are decided here — the shared validator has no opinion on them:
...
if cr, err := h.checkDefaultStorageClasses(); err != nil {
    ...
    checks["DefaultStorageClassMissing"] = *cr
}
...
```go
storageClassClient.OnChange(ctx, "component-health-sc", handler1)
virtualMachineClient.OnChange(ctx, "component-health-vm", handler2)
```
...
2. It computes per-component error and warning counts into a component-name-keyed map.
...
- [ ] Add webhook registration per component using the same `Checker` implementations.
...
`ComponentHealth` is different: any component that lives in a separate repository (e.g. `pcidevices-controller`) must write its own `ComponentHealth` CR. That means the `health.harvesterhci.io/v1beta1` API types — including `ComponentHealth`, `CheckResult`, `AffectedResources`, `AffectedResourceDetail`, and the generated client/informer code — must be importable from outside `harvester/harvester`. This introduces a cross-repository import dependency: external component repositories must vendor or depend on the Harvester API package that exposes these types.
