...
This enhancement introduces multipath device recognition and management in Harvester's node-disk-manager component, allowing users to configure and utilize multipath storage devices safely while ensuring proper identification and avoiding conflicts with existing Longhorn v2 dm-devices.
...
#### Story 1: Add multipath disk as Harvester backend storage.

**Before:** Users cannot configure Harvester with multipath storage devices, leading to single points of failure in storage connectivity and inability to use enterprise SAN storage effectively.

**Why needed:** Using multipath storage is for high availability and fault tolerance. Without multipath support, users must rely on single-path connections which create availability risks.

**After:** Users can configure multipath devices during Harvester installation and have automatic failover capabilities when storage paths fail.
...
1. **Enhanced Device Recognition:** Added support for multipath device identification by extending device property extraction to handle `DM_SERIAL` and `DM_WWN` attributes specific to multipath devices.

2. **Device Filtering:** Implemented logic to distinguish multipath devices from other dm-devices (such as Longhorn v2 volumes) using multipath command-line tools for verification.
...
For this part, I think we can open a new issue to track this and determine whether it's necessary to implement. We just need to focus on supporting multipath devices in this HEP.
