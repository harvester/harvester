...
│   ├── README.md                    # how to operate built-in / non-built-in addons
...
On top of the per-addon READMEs, a top-level `addons/README.md` guides users on
operating addons now that they live in the harvester repository: how to enable a
built-in addon, how to install a non-built-in addon from the versioned raw URL,
and a table listing every addon with its stage and packaging so the full catalog
...
4. Removal timeline: an addon deprecated in release N is removed from the
   repository in release N+1 (its directory and manifest deleted). The Addon CR
   on clusters that keep running it is still left untouched; the
   chart-availability guidance below applies from the moment of deprecation.
...
4. Repo disposition after the migration merges:
   - `harvester/experimental-addons`: archived immediately. Installing from the
     archived repository stays supported for one minor release after the new
     mechanism ships; after that it is unsupported and current documentation no
     longer references it. The frozen raw URLs themselves keep resolving, which
     the versioned docs of already-released versions depend on.
...
Addon releases follow the unified hotfix mechanism; the details will be covered
in the upcoming hotfix framework HEP.
...
- Publishing non-built-in addon manifests as release artifacts alongside the
  Harvester ISO, so users (air-gapped ones in particular) obtain them from the
  release page without reaching raw.githubusercontent.com.
