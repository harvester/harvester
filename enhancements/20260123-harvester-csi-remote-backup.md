...
### Snapshot Type Routing (Guest Cluster)
...
### Snapshot ID Format (Guest Cluster)
...
1. Harvester host cluster has `VolumeRemoteBackup` and `VolumeRemoteRestore` CRDs installed.
2. Longhorn backup target (S3-compatible or NFS) is configured and reachable.
3. Guest cluster uses the Harvester CSI driver and can provision Longhorn-backed PVCs.
4. Snapshot controller/CRDs are available in the guest cluster for CSI snapshot operations.

### Host Cluster Validation

1. VolumeRemoteBackup create and completion
   - Create a source PVC on Harvester and write known test data.
   - Create a `VolumeRemoteBackup` with `spec.source` set to the PVC name.
   - Verify `status.success=true`, `status.handle` is populated, and `status.sourceSpec` is stored.

2. VolumeRemoteBackup immutability
   - Attempt to update `VolumeRemoteBackup.spec.source` (or `spec.type`) after creation.
   - Verify the webhook rejects the update.

3. VolumeRemoteRestore create and completion
   - Create a `VolumeRemoteRestore` that references an existing backup via `spec.from: "namespace/name"`.
   - Verify restore reaches `status.success=true` and a new restored PVC is created and bound.
   - Mount the restored PVC and verify test data integrity.

4. VolumeRemoteRestore validation for missing backup
   - Create a `VolumeRemoteRestore` with `spec.from` pointing to a non-existent `VolumeRemoteBackup`.
   - Verify creation is rejected by webhook validation.

5. Deletion protection during active restore
   - Start a restore from a backup and, while restore is in progress, attempt to delete the referenced `VolumeRemoteBackup`.
   - Verify deletion is rejected until no active `VolumeRemoteRestore` depends on it.

### Guest Cluster CSI Validation

1. CreateSnapshot local path (baseline)
   - Create a PVC and call CSI `CreateSnapshot` without `type=backup`.
   - Verify a normal `VolumeSnapshot` flow is used and returned snapshot ID is local format.

2. CreateSnapshot backup path
   - Create a PVC with test data.
   - Call CSI `CreateSnapshot` with parameter `type=backup`.
   - Verify host cluster creates `VolumeRemoteBackup` and it becomes successful.
   - Verify CSI returns snapshot ID in `backup:namespace:name` format.

3. CreateVolume restore from backup snapshot ID
   - Call CSI `CreateVolume` with `VolumeContentSource` pointing to a `backup:namespace:name` snapshot ID.
   - Verify host cluster creates `VolumeRemoteRestore`, restore succeeds, and target PVC is created.
   - Attach the volume to a pod and verify restored data integrity.

4. DeleteSnapshot routing
   - Delete a local snapshot ID and verify `VolumeSnapshot` is removed.
   - Delete a backup snapshot ID (`backup:namespace:name`) and verify `VolumeRemoteBackup` is removed.

5. DeleteVolume restore cleanup
   - For a restored volume, call CSI `DeleteVolume`.
   - Verify related `VolumeRemoteRestore` is deleted before/with volume cleanup.

6. ListSnapshots unified result
   - Create at least one local snapshot and one backup snapshot.
   - Call CSI `ListSnapshots` with and without source volume filters.
   - Verify response includes both local entries and backup entries, and backup entries use `backup:namespace:name` IDs.

### Cross-Namespace and Cross-Cluster Scenarios

1. Cross-namespace restore in same guest cluster
   - Create backup from a PVC in namespace `ns-a`.
   - Restore into namespace `ns-b`.
   - Verify the underlying host-side `VolumeRemoteRestore.spec.from` references backup as `hns-a/backup-name` and restore succeeds.

2. Cross-cluster restore (same Harvester provider)
   - Create backup from guest cluster A.
   - Restore into guest cluster B using the same Harvester host.
   - Verify restored PVC is usable and data matches source.
