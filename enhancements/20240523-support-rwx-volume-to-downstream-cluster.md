...
#### 1. The Configuration Backup of NueVector
NueVector will backup the configuration files (Policies, Rules, user data, integrations, etc) in the controller. The backup needs the RWX volume to store to ensure this configuration is safe. Currently, we only support RWO volume. The user needs to wait for this feature so that they can use Nuevector in the downstream cluster.

Reference: https://open-docs.neuvector.com/deploying/production/#persistent-volume-example

#### 2. The Backup Store of MongoDB
MongoDB could configure the backup store with the Kubernetes Operator. The prerequisite is the RWX volume, which is similar to the NueVector. The user needs this feature once they want to use K8s Operator to backup MongoDB.
...
We need to create a workload pod to bring up the Longhorn-share manager so that the Longhorn-share-manager can create the NFS endpoint.
