...
- Harvester cluster with 3 nodes with `mgmt-br.2021` having the following management IPs.
...
- Create cluster network `br-1` with networkconfig spanning all 3 nodes

- br-1-br.2012 exists on all 3 nodes with the following IPs.
...
#### Story 1 Update the `HostNetworkConfig` having cluster-network `br-1` and vlan-id `2012` with `underlay` as `true`

- updates the node annoation `ovn.kubernetes.io/tunnel_interface` with `br-1-br.2012`
...
#### Story 2 Update the `HostNetworkConfig` having cluster-network `br-1` and vlan-id `2012` with `underlay` as `false`
...
#### Story 4 Update the `HostNetworkConfig` having cluster-network `br-1` and vlan-id `2014` with `underlay` as `true`

- webhook rejects the configuration as only one underlay exists for the whole cluster.
...
- Pre-requisite `mgmt-br.2014` with `ip address` exists on all nodes.
...
- HostNetworkConfig agent running on each node does the following,
  ...
  - if the underlay is updated as false, the hostnetworkconfig agent takes care of switching back to default mgmt interface's managment IPs as underlay.
...
- Webhook must reject updating `HostNetworkConfig` used as underlay if VMs or VMIs present for overlay networks.
