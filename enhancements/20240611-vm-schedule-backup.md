...
- `status.suspend` presents if the schedule is suspended
...
- ScheduleVMBackup reconcile:
  ...
  - If `.spec.suspend` is true
    - Suspend the schedule and update `.status.suspend` accordingly
  - If `.spec.suspend` is false  
    - During `status.failure` less than `spec.maxFailure`
      - Resume the schedule and update `.status.suspend` accordingly
  ...
  - Update schedule accroding to the cron expression of `spec.cron`
...
- On cronjob is scheduled according to the cron expression
  - If `status.failure` equals or more than `spec.maxFailure`
    ...
    - `.spec.suspend` and `.status.suspend` will be updated accordingly
...
1. Schedule VM backup for a running VM
   ...
   - New VM backup keeps being created in every minute
     ...
     - The oldest Longhorn snapshot should also be purged under the hood
