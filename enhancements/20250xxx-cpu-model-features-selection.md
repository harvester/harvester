...
Harvester doesn't provide the options for selecting CPU models and features for VMs. It might result in migration issue, such as forcing vm shutdown or failed upgrade. This enhancement is to provide a way to select CPU models and features for VMs.
...
- Provide a way to select a specific CPU model per VM.
- Provide a way to select specific CPU features per VM.
- Provide a way to select a specific CPU model in cluster-wide configuration.
...
I have multiple nodes that have a common CPU model called `Nehalem`. For some reason, my VMs cannot be shut down. Therefore, I want to migrate my VMs to other nodes without  manual shutdown. To ensure compatibility, I choose `Nehalem` as the CPU model.
...
![](20250xxx-cpu-model-features-selection/01-models-and-features.png)

If users select the uncommon one, we'll show the warning message.

![](20250xxx-cpu-model-features-selection/02-warning-message.png)
...
![](20250xxx-cpu-model-features-selection/03-different-model.png)

![](20250xxx-cpu-model-features-selection/04-custom-model.png)
...
- [ ] Write a document about different usage of policy field in the VM spec.
