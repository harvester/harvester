...
- Bound the node drain by a configurable timeout. The default is 15 minutes; a value of `0` disables the timeout.
...
The new `maintenance-mode-drain-timeout` setting is an integer number of minutes. Its default is `15`; `0` disables the overall drain deadline. Negative values must be rejected by setting validation. The timeout begins on entering `Draining` and bounds `DrainNode`, which waits for pod eviction and deletion, including VM evacuation.

The deadline is calculated from `Draining.LastTransitionTime` and the current setting value. A setting change therefore applies to an active drain; `0` disables its deadline. The existing four-minute kubectl drain timeout remains a per-call timeout; failures before the overall deadline may be retried.
...
### Clarification and correction of forced drain semantics

The earlier enhancement proposal [20240605-enhanced-maintenance-mode](./20240605-enhanced-maintenance-mode.md) stated that selecting `Force` in the maintenance dialog would override a VM's configured `harvesterhci.io/maintain-mode-strategy`, causing the VM to be shut down and remain off regardless of its strategy.

However, both the concept and its initial implementation were flawed:
1. **Flawed Concept:** The intended purpose of `Force` in Harvester has always been to bypass pre-checks and shut down *non-migratable* workloads (e.g. VMs with node affinities or single-replica volumes) so that node maintenance can proceed. Overriding intentionally configured restart strategies (`ShutdownAndRestartAfterEnable`, `ShutdownAndRestartAfterDisable`) was contradictory to the user's intent to manage service lifecycles across maintenance windows.
2. **Flawed Implementation:** The previous implementation in `nodedrain_controller` attempted to bypass tracking when forced was active, but did so inconsistently, leading to edge cases where VMs were stopped but lost their tracking annotations or conflicted with non-migratable VM lists.

This proposal formally supersedes that specification and clarifies the semantics:
- **`Force` applies strictly to non-migratable VMs:** It authorizes Harvester to shut down workloads detected by `FindNonMigratableVMS` that would otherwise block the drain during pre-checks.
- **Configured VM strategies are always preserved:** VMs explicitly labeled with `harvesterhci.io/maintain-mode-strategy` (including `ShutdownAndRestartAfterEnable` and `ShutdownAndRestartAfterDisable`) are shut down and retain their tracking annotations (`harvesterhci.io/maintain-mode-strategy-node-name`), ensuring they are restarted at the expected stage even when `Force` was enabled.
...
- **Error (False) → No Condition (clear):** User invokes `clearMaintenanceMode`; the side-effect-free pre-check error is removed. The user must clear it before starting a new maintenance attempt.
...
4. A pre-check failure sets `status: False`, `reason: Error`; no drain side effects have occurred. The user must clear the error before starting a new attempt. Before the drain deadline, ordinary drain errors are retried in `Draining` without resetting the deadline.
5. If the drain deadline expires, Harvester sets `status: True`, `reason: Error`, leaves cordon, taints, workloads, VM state, and migrations unchanged, and removes only the drain intent annotations. The user must disable maintenance mode before starting a new attempt.
...
It accepts a non-negative integer number of minutes. The default is `15`; `0` disables timeout enforcement. Negative values must be rejected by setting validation.
...
- **Availability:** shown only when a `MaintenanceMode` condition exists with `status: False` / `reason: Error` and intent cleanup is complete (neither `harvesterhci.io/drain-requested` nor `harvesterhci.io/drain-forced` is present). It remains unavailable while annotation cleanup is pending.
- **Permission:** same permissions as `disableMaintenanceMode`.
- **Behavior:** removes the `MaintenanceMode` condition. It does not touch intent annotations, cordon state, or VMs.
...
| `False/Error` (cleanup complete) | `clearMaintenanceMode` |
| `False/Error` (cleanup pending) | none |
...
2. **Pre-checks (reason `Validating`):**
    ...
    - If cleanup is incomplete, do not permit clearing the error or starting another attempt.
...
3. **Drain (reason `Draining`):**
    ...
    - Calculate the deadline from `Draining.LastTransitionTime` and the current `maintenance-mode-drain-timeout`. It bounds `DrainNode` (cordon + drain), not preceding VM shutdown.
...
4. Reconcile nodes whose `MaintenanceMode` condition reason is `Evacuating`:
    ...
    - Failures in this phase are retried and do not transition to `Error`. `disableMaintenanceMode` remains available.
...
**Disable:** offered for `Evacuating`, `Completed`, and `Error=True`. Perform the existing uncordon, drain-taint cleanup, intent cleanup, and disable-specific VM behavior first; remove the `MaintenanceMode` condition last through the Node status subresource using the latest state and conflict handling. Disable is not offered in `Validating` or `Draining`.

**Clear error:** offered only when the `MaintenanceMode` condition exists with `status: False` / `reason: Error` and intent cleanup is complete. Remove the condition through the Node status subresource using the latest state and conflict handling. Reject or hide the action for `Error=True`.
...
|----------|--------------------|-------------------|
| `pkg/api/node/formatter.go` | shows enable vs. disable based on annotation presence | uses the explicit phase/action matrix; gates `clearMaintenanceMode` on `status: False` after intent cleanup |
...
The trigger annotations `harvesterhci.io/drain-requested` and `harvesterhci.io/drain-forced` are unchanged and continue to express user intent. When calculating control-plane availability, `DrainPossible` excludes the candidate node and requires all remaining control-plane/etcd nodes to be available. A three-node control-plane cluster can therefore place one node into maintenance mode.
...
- Changes to the timeout setting apply to active drains; invalid values are rejected.
- `DrainNode`, but not preparatory VM shutdown, is bounded by the `Draining` deadline.
...
- Clear and Disable remove the condition through the Node status subresource while preserving concurrent changes.
