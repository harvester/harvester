...
```
The status comes from the definiation of StorageClass/CSI driver. You can define specific values by adding the spec.<field> in the StorageProfile.
...
- VM Live Migration
  - Ensure the VM can be live migrated with various disk configurations. (Like a mix of the Longhorn v1 and LVM)
    - GoTo Virtual Machines Page
...
- VM snapshot
  - Ensure the VM can take the snapshot with various disk configurations. (Like a mix of the Longhorn v1 and LVM)
    - GoTo Virtual Machines Page
    - Select the VM
