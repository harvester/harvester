...
The promote handler's `OnNodeChanged()` function looks for this label, and if it's present, sets the node to unschedulable, then adds `harvesterhci.io/install-cordoned-processed=true` as an annotation to indicate the cordon was processed, and shouldn't be performed again.
...
Validation logic will be added to the installer to prevent `install.cordon` being applied in create mode, and also to ensure this option isn't used for witness nodes. The first node in a cluster can't be cordoned, and it doesn't make sense to cordon witness nodes for the purpose of this feature, as witness nodes don't host workloads anyway.
