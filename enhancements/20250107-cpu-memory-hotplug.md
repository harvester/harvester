...
In old system, the initial CPU and Memory resources are set to `resources.limits`. We should migrate these fields to `template.spec.domain.cpu.sockets` and `template.spec.domain.memory.guest` when the VM is stopped or restarted.
