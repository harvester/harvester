...
We extend the existing `harvesterhci.io/volumeClaimTemplates` annotation on the VirtualMachine CR to support storage migration. The annotation value is a JSON array of `VolumeClaimTemplateEntry` objects. A new optional `targetVolume` field is added to each entry to indicate the target PVC for migration.
...
[
  {
    ...
    "targetVolume": "my-vol-target"
  }
]
```

- The `PersistentVolumeClaim` fields are inlined, so each entry contains the standard PVC `metadata` and `spec` directly (same as the existing format).
...
- Hotpluggable disks cannot be migrated.
- virtiofs disks are not supported as they lack live-migration capability.
- LUN disks cannot be migrated when the destination storage class differs from the source.
- Volume migration requires the VM to migrate to a different node; same-node storage migration is not supported.
- Migrating between two RWO (ReadWriteOnce) PVCs backed by local storage is only permitted when the PVCs reside on different nodes.
- Only PersistentVolumeClaims and DataVolumes are supported as migration sources.
- Unsupported disk types will receive a `RestartRequired` condition and the volumes will be replaced upon VM restart rather than during live migration.
...
When a volume migration fails in a way that requires manual intervention, KubeVirt sets a `ManualRecoveryRequired` condition on the VirtualMachine CR. UI should disaply the corresponding message to the user and we should provide documentation on how to resolve the issue and clear the condition.
...
- Create StorageMigration scenario as above and try to simulate a failure in the middle of the process.
