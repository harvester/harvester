...
If needed, user can append custom permissions to the provided role templates to extend the scope of the roles via Rancher's role "Inherit From" feature.
...
Generally, these roles do not have permissions to access infrastructure and system resources, nor project resources outside of the associated RBAC binding context.
...
* Fine-grained RBAC roles with specific permissions to manage certain resource types, to perform specific operation types, to manage networking stack or storage configuration
...
The proposed roles do not include Rancher-level administrative permissions, which grant privileges to manage Rancher resources like Rancher membership or node drivers configuration. These permissions can be granted to users from the existing "Built-In" global permissions page:
...
* modify cluster and project memberships
...
Due to the additive nature of the RBAC permissions, a project-scoped user should not be assigned Harvester cluster-scoped roles to prevent granting them excessive access to other projects they are not authorized for.
...
For UI test plan, see [Rancher Integration RBAC - UI Test Plan](./20260130-rancher-integration-rbac/test-plan-ui.md).

For automated test plan, see [Rancher Integration RBAC - Automated Test Plan](./20260130-rancher-integration-rbac/test-plan-automated.md).
