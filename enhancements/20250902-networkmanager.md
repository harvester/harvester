...
For example, if bonding options have been changed as described in [the docs], there will have been manual edits made to the `/etc/sysconfig/network/ifcfg-mgmt-bo` file in `/oem/90_custom.yaml`. Those `ifcfg-*` file aren't used by NetworkManager, so instead matching changes will need to be made to the appropriate `/etc/NetworkManager/system-connections/*.nmconnection` file(s) on each node once upgrade is complete.
...
- Add a subcommand (something like `harvester-installer generate-network-config`) to generate NetworkManager connection profiles as needed during upgrades.
...
- Pull the harvester-installer binary into the upgrade container, then run `harvester-installer generate-network-config` at an appropriate point in `package/upgrade/upgrade_node.sh` to generate a new YAML file in `/oem` that will create NetworkManager connection profiles based on the current `/oem/harvester.config`.
...
During upgrades, we will create a new file, something like `/oem/91_networkmanager.yaml`, containing only the NetworkManager connection profiles.  These will be generated based on the content of `/oem/harvester.config`, so will exactly match the network configuration from when the system was originally installed.

If the user has made manual changes to the networking config in `/oem/90_custom.yaml` after installation, they will potentially need to make similar changes to `/oem/91_networkmanager.yaml` after upgrade to have the same settings applied correctly.  Another possibility (depending on the exact nature of the changes) may be to edit `/oem/harvester.config` _before_ the upgrade, so that the change will be taken into account when generating `/oem/91_networkmanager.yaml`.  Either way this will need to be well documented.
