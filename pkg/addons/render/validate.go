...
// Validate enforces the addons/ packaging contract for every addon under
// addonsDir and returns one error per violation found (nil if none).
...
func validateAddon(addonsDir string, meta *Metadata) []error {
	...
	if _, err := os.Stat(filepath.Join(dir, readmeFileName)); err != nil {
		errs = append(errs, fmt.Errorf("%s: README.md is required", meta.Dir))
	}

	if meta.Deprecated && meta.BuiltIn {
		errs = append(errs, fmt.Errorf("%s: deprecated addons must not be builtIn", meta.Dir))
	}

	if meta.BuiltIn {
		errs = append(errs, validateBuiltIn(dir, meta)...)
	} else {
		errs = append(errs, validateNonBuiltIn(dir, meta)...)
	}
...
func validateBuiltIn(dir string, meta *Metadata) []error {
	var errs []error

	fragPath := filepath.Join(dir, builtInTemplateFile)
	contents, err := os.ReadFile(fragPath)
	if err != nil {
		return []error{fmt.Errorf("%s: builtIn addon requires %s: %w", meta.Dir, builtInTemplateFile, err)}
	}
	...
	versionFile := filepath.Join(filepath.Dir(dir), "version_info")
...
// validateNonBuiltIn validates the packaging contract for a non-built-in
// addon. The addon must provide a valid addon.yaml containing at least one
// Addon resource, and its derived labels must already match metadata.yaml
// because non-built-in manifests are not modified by the generator.
//
// For example, an addon with stage: ga must include
// addon.harvesterhci.io/ga: "true" in the Addon resource's metadata.labels.
func validateNonBuiltIn(dir string, meta *Metadata) []error {
	var errs []error

	addonPath := filepath.Join(dir, nonBuiltInAddonFile)
	contents, err := os.ReadFile(addonPath)
	if err != nil {
		return []error{fmt.Errorf("%s: non-builtIn addon requires %s: %w", meta.Dir, nonBuiltInAddonFile, err)}
	}

	docs, err := splitYAMLDocuments(string(contents))
	if err != nil {
		return []error{fmt.Errorf("%s: %s is not valid YAML: %w", meta.Dir, nonBuiltInAddonFile, err)}
	}
	...
	for _, doc := range docs {
		...
		if err := yaml.Unmarshal([]byte(doc), &res); err != nil {
			errs = append(errs, fmt.Errorf("%s: %s is not valid YAML: %w", meta.Dir, nonBuiltInAddonFile, err))
			continue
		}
		if res["kind"] != "Addon" {
			continue
	...
	if !found {
		errs = append(errs, fmt.Errorf("%s: %s does not contain an Addon resource", meta.Dir, nonBuiltInAddonFile))
	}
...
// Non-builtIn addon.yaml files are static, fully-rendered manifests with no
...
// resource (used for fully-rendered, non-builtIn addon.yaml files).
