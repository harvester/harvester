...
// writeBuiltInAddon writes a minimal built-in addon directory and returns its path,
// for validate tests that need to poke at specific drift scenarios rather than
// the real addons/ tree.
func writeBuiltInAddon(t *testing.T, root, name, stage string, fragmentLabels string) string {
	t.Helper()
	dir := filepath.Join(root, builtInCategory, name)
...
func TestValidateDeprecatedBuiltIn(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "version_info"), []byte("#!/bin/bash\n"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	errs := Validate(root)
	if len(errs) == 0 {
		t.Fatal("expected error for deprecated built-in addon, got none")
	}
}
...
func TestValidateBuiltInUnresolvedPlaceholder(t *testing.T) {
	...
	dir := filepath.Join(root, builtInCategory, "foo")
	if err := os.MkdirAll(dir, 0755); err != nil {
		t.Fatal(err)
	}
