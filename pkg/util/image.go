...
func GenerateStorageClassName(imageNamespace, imageName string) string {
	return lhutil.AutoCorrectName(fmt.Sprintf("longhorn-%s-%s", imageNamespace, imageName), lhdatastore.NameMaximumLength)
}
...
func GetStorageClass(storageClassCache ctlstoragev1.StorageClassCache, image *harvesterv1.VirtualMachineImage) (*storagev1.StorageClass, error) {
	...
	// If not, return a new format based on the image namespace and name which can avoid name conflict.
	...
	return storageClassCache.Get(GenerateStorageClassName(image.Namespace, image.Name))
}
...
func GetImageStorageClassName(storageClassCache ctlstoragev1.StorageClassCache, image *harvesterv1.VirtualMachineImage) (string, error) {
	...
	return GenerateStorageClassName(image.Namespace, image.Name), nil
}
