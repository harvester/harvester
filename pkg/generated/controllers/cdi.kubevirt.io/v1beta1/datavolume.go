...
// DataVolumeController interface for managing DataVolume resources.
type DataVolumeController interface {
	generic.ControllerInterface[*v1beta1.DataVolume, *v1beta1.DataVolumeList]
}

// DataVolumeClient interface for managing DataVolume resources in Kubernetes.
type DataVolumeClient interface {
	generic.ClientInterface[*v1beta1.DataVolume, *v1beta1.DataVolumeList]
}

// DataVolumeCache interface for retrieving DataVolume resources in memory.
type DataVolumeCache interface {
	generic.CacheInterface[*v1beta1.DataVolume]
}

// DataVolumeStatusHandler is executed for every added or modified DataVolume. Should return the new status to be updated
type DataVolumeStatusHandler func(obj *v1beta1.DataVolume, status v1beta1.DataVolumeStatus) (v1beta1.DataVolumeStatus, error)

// DataVolumeGeneratingHandler is the top-level handler that is executed for every DataVolume event. It extends DataVolumeStatusHandler by a returning a slice of child objects to be passed to apply.Apply
...
// RegisterDataVolumeStatusHandler configures a DataVolumeController to execute a DataVolumeStatusHandler for every events observed.
...
// RegisterDataVolumeGeneratingHandler configures a DataVolumeController to execute a DataVolumeGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured DataVolumeGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *dataVolumeGeneratingHandler) isNewResourceVersion(obj *v1beta1.DataVolume) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *dataVolumeGeneratingHandler) storeResourceVersion(obj *v1beta1.DataVolume) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
