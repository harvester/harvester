...
func TestHandler_OnVmimChanged_WithResourceQuota_WithCompensation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "VMIM is in Pending and with RQ Rejected status",
			args: args{
				rq: &corev1.ResourceQuota{
					ObjectMeta: v1.ObjectMeta{
						Namespace: resourceQuotaNamespace,
...
func Test_isVmiResetHarvesterMigrationAnnotationRequired(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "vmi migration is ongoing, does not require clean",
			args: args{
				vmi: &kubevirtv1.VirtualMachineInstance{
					ObjectMeta: v1.ObjectMeta{
						Name:      "vm1",
		...
		{
			name: "vmi migration is completed but MigrationUID does not match, does not require clean",
			args: args{
				vmi: &kubevirtv1.VirtualMachineInstance{
					ObjectMeta: v1.ObjectMeta{
						Name:      "vm1",
						...
						UID:       uid,
						Annotations: map[string]string{
							util.AnnotationMigrationUID:   "unrelated",
							util.AnnotationMigrationState: StatePending,
						},
					},
					Status: kubevirtv1.VirtualMachineInstanceStatus{
