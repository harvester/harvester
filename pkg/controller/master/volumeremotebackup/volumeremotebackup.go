...
func (rbh *remoteBackupHandler) handleReadinessError(
	...
	if common.IsRetryLater(err) {
		logrus.Infof("PVCBackup %s/%s not ready, retrying in 5 seconds", rbh.bo.GetNamespace(vrb), rbh.bo.GetName(vrb))
		rbh.vrbController.EnqueueAfter(rbh.bo.GetNamespace(vrb), rbh.bo.GetName(vrb), 5*time.Second)
		return vrb, nil
	}

	if apierrors.IsNotFound(err) {
		logrus.Infof("Resource not found for PVCBackup %s/%s, creating it", rbh.bo.GetNamespace(vrb), rbh.bo.GetName(vrb))
...
func (rbh *remoteBackupHandler) handleReadyBackup(vrb *harvesterv1.VolumeRemoteBackup) (*harvesterv1.VolumeRemoteBackup, error) {
	...
	updatedPb, err := rbh.bo.SetReady(vrb)
	if err != nil {
		return vrb, err
	}

	updatedPb, err = rbh.bo.SetSuccess(updatedPb, true)
	if err != nil {
		return vrb, err
	}

	updatedPb, err = rbh.bo.UpdateCSIProvider(updatedPb)
	if err != nil {
		return vrb, err
	}

	updatedPb, err = rbh.bo.UpdateSourceSpec(updatedPb)
	if err != nil {
		return vrb, err
	}
...
func (rrh *remoteRestoreHandler) handleReadinessError(
	...
	if common.IsRetryLater(err) {
		logrus.Infof("PVCRestore %s/%s not ready, retrying in 5 seconds", rrh.ro.GetNamespace(vrr), rrh.ro.GetName(vrr))
		rrh.vrrController.EnqueueAfter(rrh.ro.GetNamespace(vrr), rrh.ro.GetName(vrr), 5*time.Second)
		return vrr, nil
	}

	if apierrors.IsNotFound(err) {
		logrus.Infof("Resource not found for PVCRestore %s/%s, creating it", rrh.ro.GetNamespace(vrr), rrh.ro.GetName(vrr))
