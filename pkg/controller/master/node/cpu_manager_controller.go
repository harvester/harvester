...
func (h *cpuManagerNodeHandler) OnJobChanged(_ string, job *batchv1.Job) (*batchv1.Job, error) {
	...
	if condition.Cond(batchv1.JobComplete).IsTrue(job) {
		...
		}).Info("cpu manager job success")
		if _, err := h.nodeClient.Update(updateNode(node, updateStatus)); err != nil {
			logrus.WithField("node_name", node.Name).WithError(err).Error("failed to update node cpu manager success status")
			return nil, err
		}
	}
	...
	if condition.Cond(batchv1.JobFailed).IsTrue(job) {
		...
		logrus.WithFields(logrus.Fields{
			...
			"job_name":  job.Name,
			"policy":    updateStatus.Policy,
		}).Info("cpu manager job failed")
		if _, err := h.nodeClient.Update(updateNode(node, updateStatus)); err != nil {
			logrus.WithField("node_name", node.Name).WithError(err).Error("failed to update node cpu manager failed status")
			return nil, err
		}
	}
