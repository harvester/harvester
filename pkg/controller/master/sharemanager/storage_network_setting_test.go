...
func TestOnStorageNetworkSettingChangeEnsuresDerivedResources(t *testing.T) {
	setting := &harvesterv1beta1.Setting{
		...
		Value: `{"vlan":2011,"clusterNetwork":"mgmt","range":"172.16.0.0/24","exclude":["172.16.0.1/28"]}`,
	}
	...
	updated, err := handler.OnStorageNetworkChange("", setting)
	require.NoError(t, err)
...
func TestOnStorageNetworkSettingChangeRemovesOldDerivedResources(t *testing.T) {
	setting := &harvesterv1beta1.Setting{
		ObjectMeta: metav1.ObjectMeta{
			Name: settings.StorageNetworkName,
			Annotations: map[string]string{
				util.NadStorageNetworkAnnotation:    "harvester-system/storagenetwork-new",
				util.OldNadStorageNetworkAnnotation: "harvester-system/storagenetwork-old",
			},
		},
		Value: `{"vlan":2011,"clusterNetwork":"mgmt","range":"172.16.0.0/24","exclude":["172.16.0.1/28"]}`,
	}
	...
	clientset := fakeclientset.NewSimpleClientset(setting, oldExcludePool)
	_, err := clientset.K8sCniCncfIoV1().NetworkAttachmentDefinitions(util.HarvesterSystemNamespaceName).Create(context.Background(), sourceNAD, metav1.CreateOptions{})
	require.NoError(t, err)
	...
	_, err = handler.OnStorageNetworkChange("", setting)
	require.NoError(t, err)
