...
func (h *VMController) SetTargetVolumeStrategy(_ string, vm *kubevirtv1.VirtualMachine) (*kubevirtv1.VirtualMachine, error) {
	...
	for i, vol := range vmCopy.Spec.Template.Spec.Volumes {
		...
		vmCopy.Spec.Template.Spec.Volumes[i].VolumeSource = h.buildVolumeSource(vm.Namespace, newName, hotpluggable)
	}
	...
	if reflect.DeepEqual(vm, vmCopy) {
		return h.handleMigrationWait(vm)
	}
...
func (h *VMController) buildVolumeSource(namespace, name string, hotpluggable bool) kubevirtv1.VolumeSource {
	_, dvErr := h.dataVolumeClient.Get(namespace, name, metav1.GetOptions{})
	if dvErr == nil {
...
func (h *VMController) handleMigrationWait(vm *kubevirtv1.VirtualMachine) (*kubevirtv1.VirtualMachine, error) {
