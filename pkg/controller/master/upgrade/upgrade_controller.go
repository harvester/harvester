...
func (h *upgradeHandler) OnChanged(_ string, upgrade *harvesterv1.Upgrade) (*harvesterv1.Upgrade, error) {
	...
	if harvesterv1.UpgradeCompleted.GetStatus(upgrade) == "" {
		// this label is set by UI when user clicks `dismiss`, the upgrade object is kept bu status is reset
		if upgrade.Labels[util.LabelUpgradeReadMessage] == "true" {
			logrus.Infof("upgrade %v/%v has %v label, no further processing", util.LabelUpgradeReadMessage, upgrade.Namespace, upgrade.Name)
			return upgrade, nil
		}
