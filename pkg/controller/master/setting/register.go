...
func Register(ctx context.Context, management *config.Management, options config.Options) error {
	...
	resourceQuota := management.HarvesterCoreFactory.Core().V1().ResourceQuota()
	controller := &Handler{
	...
	syncers = map[string]syncerFunc{
		...
		"patch-harvester-services-in-resource-quota": controller.syncPatchHarvesterServicesInResourceQuota,
