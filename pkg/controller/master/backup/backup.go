...
func (h *Handler) initBackup(backup *harvesterv1.VirtualMachineBackup, vm *kubevirtv1.VirtualMachine) error {
	...
	backup.Status.ReadyToUse = ptr.To(false)
	backup.Status.SourceUID = ptr.To(vm.UID)
	backup.Status.SourceSpec = &harvesterv1.VirtualMachineSourceSpec{
		ObjectMeta: metav1.ObjectMeta{
...
func (h *Handler) setStatusError(vmBackup *harvesterv1.VirtualMachineBackup, err error) error {
	...
	harvesterv1.BackupConditionProgressing.Message(vmBackupCpy, err.Error())
