...
func (v *namespaceValidator) validatePSSOnNamespaceUpdate(req *types.Request, oldNamespace, newNamespace *corev1.Namespace) error {
	...
	whitelistedNamespacesList := strings.Split(pssSetting.WhitelistedNamespacesList, ",")

	// namespae is part of whitelistedNamespacesList, so ignore changes to this
	if slices.Contains(whitelistedNamespacesList, oldNamespace.Name) {
		return nil
	}
