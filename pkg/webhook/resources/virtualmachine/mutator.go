...
func (m *vmMutator) checkDHCPEnabledOnSubnet(namespace, name string) (error, bool) {
	provider := fmt.Sprintf("%s.%s.ovn", name, namespace)

	if m.kubeovnSubnet == nil {
		return fmt.Errorf("kubeovn subnet cache is not initialized"), false
	}
...
func (m *vmMutator) getNadAndDHCPOption(networkName string, vm *kubevirtv1.VirtualMachine) (bool, error) {
	...
	case 2:
		namespace, name = words[0], words[1]
	default:
		return enabled, fmt.Errorf("invalid network name %s", networkName)
	}
	...
	if subnetErr, enabled = m.checkDHCPEnabledOnSubnet(namespace, name); subnetErr != nil {
