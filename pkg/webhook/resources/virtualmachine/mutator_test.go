...
func TestPatchSataCdRomHotpluggable(t *testing.T) {
	type patch struct {
		Op    string       `json:"op"`
		Path  string       `json:"path"`
	...
	tests := []struct {
		...
		},
		{
			...
			patchOps: []patch{
				{
					Op: "replace",
					Path: "/spec/template/spec/volumes/1/persistentVolumeClaim/hotpluggable",
					Value: true,
				},
			},
		},
		{
			name: "with empty cdrom",
			disks: []kubevirtv1.Disk{
				{
					Name: "cdrom1",
					DiskDevice: kubevirtv1.DiskDevice{
		...
		{
			name: "with cdrom already hotpluggable",
			disks: []kubevirtv1.Disk{
				{
					Name: "cdrom1",
					DiskDevice: kubevirtv1.DiskDevice{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// act
			actualPatchOps, err := mutator.(*vmMutator).patchSataCdRomHotpluggable(vm, nil)
