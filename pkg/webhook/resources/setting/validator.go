...
func validateUpgradeConfigFields(setting *v1beta1.Setting) error {
	...
	// If the user set LogReadyTimeout to a value not in the permissible range [1-20 minutes], we return an error
	if timeoutDuration < 1*time.Minute || timeoutDuration > 20*time.Minute {
		return fmt.Errorf("invalid logReadyTimeout must be between 1 to 20 minutes, given: %d", timeoutDuration)
	}
