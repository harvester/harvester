...
func (v *virtualMachineBackupValidator) checkTotalSnapshotSize(vm *kubevirtv1.VirtualMachine) error {
	...
	if resourceQuota.Spec.SnapshotLimit.VMTotalSnapshotSizeQuota != nil {
		if err := v.checkTotalSnapshotSizeOnVM(vm, resourceQuota.Spec.SnapshotLimit.VMTotalSnapshotSizeQuota[vm.Name]); err != nil {
			return err
		}
	}
...
func (v *virtualMachineBackupValidator) checkTotalSnapshotSizeOnVM(vm *kubevirtv1.VirtualMachine, totalSnapshotSizeQuota int64) error {
	...
	for _, volume := range vm.Spec.Template.Spec.Volumes {
		...
		engine, err := webhookutil.GetLHEngine(v.engineCache, pvc.Spec.VolumeName)
		if err != nil {
