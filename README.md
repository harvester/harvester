...
- Networking and service mesh options: Istio, Linkerd, Cilium, Service, Ingress, NetworkPolicy.
...
- Wizard-driven workload configuration
- Storage selection for local, NFS, SMB, Ceph, NVMe-oF, RDMA, ZFS, iSCSI, GlusterFS, Longhorn, OpenEBS, Portworx
- Auto-generated `Deployment` / `StatefulSet` / `DaemonSet` / `Job` / `CronJob`
...
This prototype is intended as a front-end Nexus demo for Harvester-style workload generation. After launching the app locally, you can use the wizard to select storage, networking, security, monitoring, GitOps, and multi-cluster options, then review generated manifests in the built-in YAML editor.
