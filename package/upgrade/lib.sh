...
upgrade_addon_rancher_logging_with_patch_eventrouter_image()
{
  ...
  if [ $EXIT_CODE != 0 ]; then
    ...
    if [[ "rancher/harvester-eventrouter:v1.5.1" > $tag ]]; then
      echo "eventrouter image is $tag, will patch to v1.5.1"
      fixeventrouter=true
    else
  ...
  if [[ $fixeventrouter == true ]]; then
    yq -e '.eventTailer.workloadOverrides.containers[0].image = "rancher/harvester-eventrouter:v1.5.1"' -i $valuesfile
  fi
