/*
Copyright The Longhorn Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by applyconfiguration-gen. DO NOT EDIT.

package v1beta2

import (
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
)

// BackupBackingImageSpecApplyConfiguration represents a declarative configuration of the BackupBackingImageSpec type for use
// with apply.
type BackupBackingImageSpecApplyConfiguration struct {
	BackingImage     *string           `json:"backingImage,omitempty"`
	BackupTargetName *string           `json:"backupTargetName,omitempty"`
	SyncRequestedAt  *v1.Time          `json:"syncRequestedAt,omitempty"`
	UserCreated      *bool             `json:"userCreated,omitempty"`
	Labels           map[string]string `json:"labels,omitempty"`
}

// BackupBackingImageSpecApplyConfiguration constructs a declarative configuration of the BackupBackingImageSpec type for use with
// apply.
func BackupBackingImageSpec() *BackupBackingImageSpecApplyConfiguration {
	return &BackupBackingImageSpecApplyConfiguration{}
}

// WithBackingImage sets the BackingImage field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the BackingImage field is set to the value of the last call.
func (b *BackupBackingImageSpecApplyConfiguration) WithBackingImage(value string) *BackupBackingImageSpecApplyConfiguration {
	b.BackingImage = &value
	return b
}

// WithBackupTargetName sets the BackupTargetName field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the BackupTargetName field is set to the value of the last call.
func (b *BackupBackingImageSpecApplyConfiguration) WithBackupTargetName(value string) *BackupBackingImageSpecApplyConfiguration {
	b.BackupTargetName = &value
	return b
}

// WithSyncRequestedAt sets the SyncRequestedAt field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the SyncRequestedAt field is set to the value of the last call.
func (b *BackupBackingImageSpecApplyConfiguration) WithSyncRequestedAt(value v1.Time) *BackupBackingImageSpecApplyConfiguration {
	b.SyncRequestedAt = &value
	return b
}

// WithUserCreated sets the UserCreated field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the UserCreated field is set to the value of the last call.
func (b *BackupBackingImageSpecApplyConfiguration) WithUserCreated(value bool) *BackupBackingImageSpecApplyConfiguration {
	b.UserCreated = &value
	return b
}

// WithLabels puts the entries into the Labels field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, the entries provided by each call will be put on the Labels field,
// overwriting an existing map entries in Labels field with the same key.
func (b *BackupBackingImageSpecApplyConfiguration) WithLabels(entries map[string]string) *BackupBackingImageSpecApplyConfiguration {
	if b.Labels == nil && len(entries) > 0 {
		b.Labels = make(map[string]string, len(entries))
	}
	for k, v := range entries {
		b.Labels[k] = v
	}
	return b
}
